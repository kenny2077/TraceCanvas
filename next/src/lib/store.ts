...
import { deleteTaskRuns } from "@/lib/history/db";
import { saveVersion } from "@/lib/history/repository";
  ...
  persist(
    (set, get) => ({
      ...
      commitBaseFor: (taskId) => {
        ...
        // baseline, then archive it to IndexedDB with full pipeline metadata
        // so the history pane shows verification scores and repair summaries.
        type Snap = {
          content: string;
          html: string;
          stats: RunStats;
        ...
        if (snap && snap.html) {
          // Compute pipeline data synchronously — validation and repair
          // are fast enough to run inline during commit.
          let verificationReport;
          let repairResult;
          let analysisSummary;
          try {
            const validation = validateHtml(snap.html);
            ...
            const passCount =
              validation.issues.length - failCount - warnCount;
            const total = Math.max(1, validation.issues.length || 3);
            verificationReport = {
              passed: failCount === 0,
              checks: validation.issues.map((i) => ({
                id: i.kind,
                label: i.kind,
                status:
                  i.severity === "error"
          ...
          try {
            repairResult = repairHtml(snap.html);
          } catch {
            // repair is best-effort
          }
          try {
            const summary = summarizeForAgent(snap.content);
            ...
            analysisSummary = {
              ...
              rowCount: structured?.rows?.length ?? 0,
            };
          } catch {
