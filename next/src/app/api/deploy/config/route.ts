...
import { validateJsonRequest, badRequest, DeployConfigRequestSchema } from "@/lib/validation/schemas";
...
export async function PUT(req: NextRequest) {
  try {
    const providerId = readProvider(req);
    const parsed = await validateJsonRequest<Partial<DeployConfig>>(req, DeployConfigRequestSchema);
    if (!parsed.ok) return badRequest(parsed.errors);
    const updated = await writeDeployConfig(providerId, parsed.value);
