...
import { validateJsonRequest, badRequest, DraftRequestSchema } from "@/lib/validation/schemas";
...
export async function POST(req: NextRequest) {
  const parsed = await validateJsonRequest<Body>(req, DraftRequestSchema);
  if (!parsed.ok) return badRequest(parsed.errors);
  const { agent, instruction, context = "", model, binOverride } = parsed.value;
