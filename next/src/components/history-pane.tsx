...
function HistoryCard({
  ...
  const score = run.verificationReport?.score;
  const repairChanged = run.repairResult?.changed;
  ...
  return (
    ...
    >
      ...
      {/* Pipeline metadata badges (0.4.0) */}
      {(score !== undefined || repairChanged) && (
        <div className="mt-1 flex items-center gap-1.5">
          {score !== undefined && (
            <span
              className="rounded px-1.5 py-0.5 text-[9px] font-mono tabular-nums"
              style={{
                background:
                  score >= 90
                    ? "rgba(47,125,58,0.1)"
